...
const data = require("../config/stockData");
...
exports.getStockHistoricData = async (req, res) => {
  try {
    ...
    const url = `https://api.tiingo.com/tiingo/daily/${req.params.ticker}/prices?startDate=${year}-${month}-${day}&token=${token}`;
...
exports.getRandomStockData = async (req, res) => {
  try {
    ...
    const url = `https://api.tiingo.com/tiingo/daily/${stock.ticker}/prices?startDate=${year}-${month}-${day}&token=${token}`;
